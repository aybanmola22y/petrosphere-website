...
import type { CompanyNewsItem, CompanyNewsCategory } from "@/data/companyNews";
...
function parseArticleParagraphs(html: string): string[] {
  ...
  const paragraphs = Array.from(content.matchAll(/<p[^>]*>([\s\S]*?)<\/p>/gi))
    ...
    .filter((t) => t.length > 0)
    // drop paragraphs that are basically image-only leftovers
    .filter((t) => !/^data:image/i.test(t));
...
export async function fetchPetrosphereNewsArticleByUrl(sourceUrl: string): Promise<CompanyNewsItem> {
  ...
  const body = parseArticleParagraphs(html);
  const summary = (body[0] ?? "").slice(0, 220) || "Read the full update on Petrosphere’s official website.";
...
export async function fetchPetrosphereLatestNews(limit = 6): Promise<CompanyNewsItem[]> {
  ...
  return items.slice(0, limit).map((p) => {
    ...
    return {
      ...
      imageSrc: p.imageSrc || "/logos/petrosphere.svg",
      body: [],
